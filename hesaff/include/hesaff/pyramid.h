...
{
  public:
    virtual void onHessianKeypointDetected(const cv::Mat &blur, float x,
                                           float y, float s,
                                           float pixelDistance, int type,
                                           float response) = 0;
};
...
struct HessianDetector
{
  ...
  public:
    ...
    HessianDetector(const PyramidParams &par)
        : edgeScoreThreshold((par.edgeEigenValueRatio + 1.0f) *
                             (par.edgeEigenValueRatio + 1.0f) /
                             par.edgeEigenValueRatio),
          // thresholds are squared, response of det H is proportional to square
          // of derivatives!
  ...
  protected:
    void detectOctaveKeypoints(const cv::Mat &firstLevel, float pixelDistance,
