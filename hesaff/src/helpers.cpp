...
template <typename ValueType> void swap(ValueType *a, ValueType *b)
{
    ValueType tmp = *a;
    *a = *b;
    *b = tmp;
...
void rectifyAffineTransformationUpIsUp(float &a11, float &a12, float &a21,
                                       float &a22)
{
...
void computeGaussMask(Mat &mask)
{
    ...
    for (int i = 1; i < endSize; i++)
        tmp[halfSize - i] +=
            exp((float((i + halfSize) * (i + halfSize)) / scale2));
...
void computeCircularGaussMask(Mat &mask)
{
    ...
    for (int i = 0; i < mask.rows; i++)
        ...
        {
            disq = float((i - halfSize) * (i - halfSize) +
...
bool interpolateCheckBorders(const Mat &im, float ofsx, float ofsy, float a11,
                             float a12, float a21, float a22, const Mat &res)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (floor(imx) <= 0 || floor(imy) <= 0 || ceil(imx) >= width ||
            ceil(imy) >= height)
            return true;
    }
    return false;
...
bool interpolate(const Mat &im, float ofsx, float ofsy, float a11, float a12,
                 float a21, float a22, Mat &res)
{
    bool ret = false;
    ...
    for (int j = -halfHeight; j <= halfHeight; ++j)
    {
        ...
        for (int i = -halfWidth; i <= halfWidth; ++i)
        {
            ...
            if (x >= 0 && y >= 0 && x < width && y < height)
            {
                ...
                *out++ = (1.0f - wy) * ((1.0f - wx) * im.at<float>(y, x) +
                                        wx * im.at<float>(y, x + 1)) +
                         (wy) * ((1.0f - wx) * im.at<float>(y + 1, x) +
                                 wx * im.at<float>(y + 1, x + 1));
            }
            else
            {
                *out++ = 0;
...
void photometricallyNormalize(Mat &image, const Mat &binaryMask, float &sum,
                              float &var)
{
...
void gaussianBlurInplace(Mat &inplace, float sigma)
{
    ...
    if (size % 2 == 0)
        size++;
    GaussianBlur(inplace, inplace, Size(size, size), sigma, sigma,
                 BORDER_REPLICATE);
}
...
Mat doubleImage(const Mat &input)
{
    ...
    for (int r = 0; r < input.rows - 1; r++)
        for (int c = 0; c < input.cols - 1; c++)
        {
            ...
            n.at<float>(r2 + 1, c2 + 1) =
                0.25f * (in[0] + in[1] + in[input.step] + in[input.step + 1]);
            ++in;
        }
    for (int r = 0; r < input.rows - 1; r++)
    {
        ...
        n.at<float>(r2 + 1, c2) =
            0.5f * (input.at<float>(r, input.cols - 1) +
                    input.at<float>(r + 1, input.cols - 1));
    }
    for (int c = 0; c < input.cols - 1; c++)
    {
        ...
        n.at<float>(r2, c2 + 1) =
            0.5f * (input.at<float>(input.rows - 1, c) +
                    input.at<float>(input.rows - 1, c + 1));
    }
    n.at<float>(n.rows - 1, n.cols - 1) =
        n.at<float>(input.rows - 1, input.cols - 1);
    return n;
}
