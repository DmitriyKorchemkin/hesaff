...
void HessianDetector::localizeKeypoint(int r, int c, float curScale,
                                       float pixelDistance)
{
    ...
    for (int iter = 0; iter < 5; iter++)
    {
        ...
        float dxx = cur.at<float>(r, c - 1) - 2.0f * cur.at<float>(r, c) +
                    cur.at<float>(r, c + 1);
        float dyy = cur.at<float>(r - 1, c) - 2.0f * cur.at<float>(r, c) +
                    cur.at<float>(r + 1, c);
        float dss = low.at<float>(r, c) - 2.0f * cur.at<float>(r, c) +
                    high.at<float>(r, c);

        float dxy =
            0.25f * (cur.at<float>(r + 1, c + 1) - cur.at<float>(r + 1, c - 1) -
                     cur.at<float>(r - 1, c + 1) + cur.at<float>(r - 1, c - 1));
        // check edge like shape of the response function in first iteration
        if (0 == iter)
        {
            float edgeScore =
                (dxx + dyy) * (dxx + dyy) / (dxx * dyy - dxy * dxy);
            if (edgeScore >= edgeScoreThreshold || edgeScore < 0)
                // local neighbourhood looks like an edge
                return;
        }
        float dxs =
            0.25f * (high.at<float>(r, c + 1) - high.at<float>(r, c - 1) -
                     low.at<float>(r, c + 1) + low.at<float>(r, c - 1));
        float dys =
            0.25f * (high.at<float>(r + 1, c) - high.at<float>(r - 1, c) -
                     low.at<float>(r + 1, c) + low.at<float>(r - 1, c));
        ...
        // if we are off by more than MAX_SUBPIXEL_SHIFT, update the position
        // and iterate again
    ...
    if (fabs(b[0]) > 1.5 || fabs(b[1]) > 1.5 || fabs(b[2]) > 1.5 ||
        fabs(val) < finalThreshold || octaveMap.at<unsigned char>(r, c) > 0)
        return;
    ...
    if (hessianKeypointCallback)
        hessianKeypointCallback->onHessianKeypointDetected(
            prevBlur, pixelDistance * (c + b[0]), pixelDistance * (r + b[1]),
            pixelDistance * scale, pixelDistance, type, val);
}
    ...
    for (int r = par.border; r < (rows - par.border); r++)
    {
        for (int c = par.border; c < (cols - par.border); c++)
        {
            ...
            if ((val > positiveThreshold &&
                 (isMax(val, cur, r, c) && isMax(val, low, r, c) &&
                  isMax(val, high, r, c))) ||
                (val < negativeThreshold &&
                 (isMin(val, cur, r, c) && isMin(val, low, r, c) &&
                  isMin(val, high, r, c))))
                                            ...
                                            Mat &nextOctaveFirstLevel)
{
    ...
    {
        // compute the increase necessary for the next level and compute the
        // next level
...
void HessianDetector::detectPyramidKeypoints(const Mat &image)
{
    ...
    {
        float sigma =
            sqrt(par.initialSigma * par.initialSigma - curSigma * curSigma);
