...
void SIFTDescriptor::precomputeBinsAndWeights()
{
    ...
    // maps every pixel in the patch 0..patch_size-1 to appropriate spatial bin
    // and weight
...
void SIFTDescriptor::samplePatch()
{
    for (int r = 0; r < par.patchSize; ++r)
    {
        ...
        for (int c = 0; c < par.patchSize; ++c)
        {
            ...
            // ori from atan2 is in range <-pi,pi> so add 2*pi to be surely
            // above zero
            const float o = float(par.orientationBins) *
                            (ori.at<float>(r, c) + 2 * M_PI) / (2 * M_PI);
...
void SIFTDescriptor::computeSiftDescriptor(Mat &patch)
{
    ...
    // photometrically normalize with weights as in SIFT gradient magnitude
    // falloff
    float mean, var;
